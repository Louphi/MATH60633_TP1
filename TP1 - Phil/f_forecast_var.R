...
f_backtest_var_rolling <- function(time_series_data, window_size = 1000, forecast_steps = 1000, confidence_level = 0.95) {
  # Check if the time series is long enough
  if (length(time_series_data) < window_size + forecast_steps) {
    stop("Time series data is not long enough for the specified window size and forecast steps.")
  }
